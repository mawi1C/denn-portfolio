...
// Victory sayings when a user beats the high score
...
export default function TypingTest() {
  ...
  const [highScore, setHighScore] = useState(112) // Default to your WPM
  ...
  // Check and update high score when finished
  useEffect(() => {
    if (isFinished && !endTime) {
      ...
      if (currentWpm > highScore) {
        ...
        setIsNewRecord(true)
        updateHighScore(currentWpm) // Update Supabase
      } else {
  ...
  // Keyboard shortcut: Ctrl/Cmd + G to toggle, Esc to close
  useEffect(() => {
  ...
  const handleRestart = useCallback(() => {
    ...
    setStartTime(null)
    setEndTime(null)
    setIsNewRecord(false) // Reset record state
    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="hidden md:flex fixed bottom-18 right-6 items-center gap-2 bg-gray-900 border border-white/10 text-gray-300 text-xs font-mono px-4 py-2 rounded-full shadow-lg hover:text-white transition z-30"
        >
          Typing test
          <span className="text-gray-500">Ctrl + G</span>
        </button>
      )}
      ...
      >
        <div className="max-w-xl w-full mx-4 p-6 rounded-2xl border border-white/10 bg-gray-900 shadow-2xl">
          ...
          <input
            ...
            className="w-full bg-gray-800 text-white font-mono text-sm rounded-lg px-3 py-2 outline-none disabled:opacity-50"
          />

          {isFinished ? (
            <div className="mt-4 p-4 border border-dashed border-white/10 rounded-xl bg-gray-950/30">
              <div className="flex items-center justify-between mb-3 pb-3 border-b border-white/5">
                <div className="flex gap-4 font-mono text-sm">
                  <span className={`font-bold ${isNewRecord ? 'text-yellow-400' : 'text-white'}`}>{wpm} WPM</span>
                  <span className="text-gray-400">{accuracy}% accuracy</span>
                </div>
                <button
                  onClick={handleRestart}
                  className="text-xs font-mono text-gray-400 hover:text-white flex gap-1 items-center"
                >
                  go again ↻
                </button>
              </div>
              ...
              <div className="font-mono text-xs text-center pt-1">
                {isNewRecord ? (
                  <div className="text-yellow-400 flex flex-col gap-1.5 items-center">
                    <span className="text-xl">🏆 NEW RECORD! 🏆</span>
                    <span className="text-white bg-gray-800 px-3 py-1 rounded-full">{wpm} WPM</span>
                    <span className="italic text-gray-200">
                      "{victorySayings[Math.floor(Math.random() * victorySayings.length)]}"
                    </span>
                  </div>
                ) : (
                  <div className="text-red-400 flex flex-col gap-1 items-center">
                    <span>You didn't win against den john's current record...</span>
                    <span className="text-white bg-gray-800 px-3 py-1 rounded-full mt-1.5">{highScore} WPM</span>
            ...
            <div className="mt-4 flex items-center justify-center pt-2">
              <span className="text-xs font-mono text-gray-600">Current High Score: <span className="text-gray-400">{highScore} WPM</span></span>
            </div>
          )}
        </div>
      </div>
